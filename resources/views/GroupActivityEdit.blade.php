...
@section('content')
    ...
    <div class="mt-10 max-w-sm">
        ...
        <p class="text-center text-gray-700 text-xl font-semibold mb-5">
            Grupinės veiklos "{{$activity[0]->title}}" redagavimas
        </p>
        <form method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-6">
                <label for="base-input" class=" mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Pavadinimas</label>
                <input value="{{$activity[0]->title}}" name="title" type="text" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500  w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                @error('title')
            ...
            <div class="mb-6">
                ...
                <input value="{{$activity[0]->size}}" name="size" type="number" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500  w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                @error('size')
                <p class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid mb-2 grid-cols-2 border border-black rounded-lg">
                @foreach($activity[0]->photos as $photo)
                    <div class="flex">
                        @if($activity[0]->photos->count() > 1)
